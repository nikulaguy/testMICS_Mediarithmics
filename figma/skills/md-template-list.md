...
- **Le déclencheur d'un filtre multi-sélection porte le NOM de la dimension, jamais sa valeur** : « Status », « Label », quel que soit l'état. Le contenu se lit dans le panneau ouvert et dans la barre de rappel. Statut : dropdown à `Dropdown / Checkbox Item` + pied `Dropdown / Clear`. Label : le déclencheur porte une **loupe** dès l'état fermé (il annonce la recherche qu'il ouvre) et se déploie au clic en champ « Search a label » ; la liste se filtre à la frappe, chaque choix part en chip et le champ reste ouvert pour cumuler.
