...
- **Rappel des filtres appliqués** : `Active Filter Bar` (Set 712:132327), posée SOUS la toolbar et AVANT le tableau. Obligatoire dès qu'un filtre actif n'est pas lisible dans la toolbar — c'est toujours le cas du modèle panneau (bouton Filters unique) et des filtres de colonne. Sur le modèle exposé, elle ne porte que les dimensions dont le sélecteur ne montre plus la valeur (ex. le filtre par label des Campaigns, dont le champ redevient un bouton après choix). Aucun filtre actif → la barre n'existe pas, pas de hauteur réservée. Voir /md-data-display.
